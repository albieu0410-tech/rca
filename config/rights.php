...
{
	if(cache_getvalue("multiwindow")!="" && strpos($newlink,"&sm=".cache_getvalue("multiwindow"))===false)
	{
		$newlink.="&sm=".cache_getvalue("multiwindow");
	}
...
function session_getvalue($name)
{
	...
	$name=(isset($_CONFIG['session_master_old'])?$_CONFIG['session_master_old']:'')."asi".$oldname;
	if(isset($_SESSION[$name]))
		return $_SESSION[$name];
...
function getDefaultValue($name,$control_name,$default,$reset=true)
{
	...
	if(session_getvalue($sess)=='')
	{
		...
		if(strpos($default,"config.")!==false)
		{
			strtok($default,".");
			$sess=strtok(".");

			return getUserConfig($sess);
		}
...
function matches3($matches) {
	$ret='&#'.((ord($matches[1])-224)*4096+(ord($matches[2])-128)*64+(ord($matches[3])-128)).";";
	return $ret;
}
function matches2($matches) {
	$ret='&#'.((ord($matches[1])-192)*64+(ord($matches[2])-128)).';';
	return $ret;
}
...
{
	$string=correctPostHtmlEntites($string);
...
function getValueForPath($value)
{
...
function getSiteConfigs()
{
	...
	if(getUserConfig("activate_multiwindow")=="yes")
	{
		if(isset($_GET['sm']) && $_GET['sm']!="")
		{
			cache_setvalue("multiwindow",intval($_GET['sm']));
			$_CONFIG['session_master_old']=$_CONFIG['session_master'];
			$_CONFIG['session_master']=$_CONFIG['session_master']."_sm_".$_GET['sm']."_";
		}
		else
		{
			session_setvalue("multiwindow",intval(session_getvalue("multiwindow"))+1);
			cache_setvalue("multiwindow",session_getvalue("multiwindow"));
			$_CONFIG['session_master_old']=$_CONFIG['session_master'];
			$_CONFIG['session_master']=$_CONFIG['session_master']."_sm_".cache_getvalue("multiwindow")."_";
		}
	}
