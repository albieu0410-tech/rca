...
function substrdate($s,$ss,$sl)
{
	$s=trim($s);
	if($sl==4)
	{
		return str_pad(intval(substr($s,$ss,4)),4,"0",STR_PAD_LEFT);
	}
	return str_pad(intval(substr($s,$ss,2)),2,"0",STR_PAD_LEFT);
}
function showDate($date,$format='')
{
	...
	if($date!="0000-00-00")
	{
		if($date!="0000-00-00 00:00:00")
		{
			if(substr($date,4,1)!="-")
			{
				$date=substrdate($date,0,4)."-".substrdate($date,4,2)."-".substrdate($date,6,2);
			}
			...
			switch($format)
			{
				...
				case 'dd/MM/yyyy':
					$ra=explode(" ", $date);
					$ta=explode("-", $ra[0]);
					return substrdate($ta[2],0,2)."/".substrdate($ta[1],0,2)."/".substrdate($ta[0],0,4);
				break;
				case '%m/%e/%Y':
				case 'monthly':
				case 'mm':
				...
				case 'MM/dd/yyyy':
					$ra=explode(" ", $date);
					$ta=explode("-", $ra[0]);
					return substrdate($ta[1],0,2)."/".substrdate($ta[2],0,2)."/".substrdate($ta[0],0,4);
				break;
				case '%e.%m.%Y':
				case 'daily':
				case 'dd':
	...
	if($nosecs=='')
	{
		...
		if($pm!="")
		{
			if(intval($hourArr[0])>12)
				$ret=substrdate(intval($hourArr[0])-12,0,2).":".substrdate($hourArr[1],0,2)." PM";
			else
				$ret=substrdate($hourArr[0],0,2).":".substrdate($hourArr[1],0,2)." AM";
		}
		else
		{
			$ret=substrdate($hourArr[0],0,2).":".substrdate($hourArr[1],0,2);
		}
	}
	else
	{
...
function showDateTime($date,$format,$nosecs='')
{
	...
	if($date!="0000-00-00")
	{
		if($date!="0000-00-00 00:00:00")
		{
			if(substr($date,4,1)!="-")
			{
				$date=substrdate($date,0,4)."-".substrdate($date,4,2)."-".substrdate($date,6,2)." ".substrdate($date,8,2).":".substrdate($date,10,2).":".substrdate($date,12,2);
			}
			if($format=='')
				$format='daily';
			switch($format)
			{
				...
				case 'dd/MM/yyyy':
					...
					$ret=substrdate($dateArr[2],0,2)."/".substrdate($dateArr[1],0,2)."/".substrdate($dateArr[0],0,4);
					if($nosecs=='')
					{
						$hourArr=explode(":", $indate[1]);
						$ret.=" ".substrdate($hourArr[0],0,2).":".substrdate($hourArr[1],0,2);
				...
				case 'MM/dd/yyyy':
					$indate=explode(" ", $date);
					$dateArr=explode("-", $indate[0]);
					$ret=substrdate($dateArr[1],0,2)."/".substrdate($dateArr[2],0,2)."/".substrdate($dateArr[0],0,2);
					if($nosecs=='')
					{
						$hourArr=explode(":", $indate[1]);
						$ret.=" ".substrdate($hourArr[0],0,2).":".substrdate($hourArr[1],0,2);
...
function getDateForMysql($date,$format='')
{
	...
	if(''.$date!='')
	{
		switch($format)
		{
			...
			case 'dd/MM/yyyy':
				$ta=explode("/", $date);
				if(count($ta)==1) {$ta[0]=substr($date,0,2);$ta[1]=substr($date,2,2);$ta[2]=substr($date,4);}
				if(intval(trim($ta[2]))<100) $ta[2]="".(2000+trim($ta[2]));
				return substrdate($ta[2],0,4)."-".substrdate($ta[1],0,2)."-".substrdate($ta[0],0,2);
			break;
			case '%m/%e/%Y':
			case 'MM/dd/yyyy':
				$ta=explode("/", $date);
				if(count($ta)==1) {$ta[0]=substr($date,0,2);$ta[1]=substr($date,2,2);$ta[2]=substr($date,4);}
				if(intval(trim($ta[2]))<100) $ta[2]="".(2000+trim($ta[2]));
				return substrdate($ta[2],0,4)."-".substrdate($ta[0],0,2)."-".substrdate($ta[1],0,2);
			break;
			case '%e.%m.%Y':
			case 'dd.MM.yyyy':
				$ta=explode(".",$date);
...
function getDateTimeForMysql($date,$format)
{
	...
	switch($format)
	{
		...
		case 'dd/MM/yyyy':
			$t=explode(" ", $date);
			$ta=explode("/", $t[0]);
			$ret=substrdate($ta[2],0,4)."-".substrdate($ta[1],0,2)."-".substrdate($ta[0],0,2)." ".trim($t[1]);
			return $ret;
		break;
		case '%m/%e/%Y':
		case 'MM/dd/yyyy':
			$t=explode(" ", $date);
			$ta=explode("/", $t[0]);
			$ret=substrdate($ta[2],0,4)."-".substrdate($ta[0],0,2)."-".substrdate($ta[1],0,2)." ".trim($t[1]);
			return $ret;
		break;
		case '%e.%m.%Y':
		case 'dd.MM.yyyy':
