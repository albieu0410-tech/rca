				...
				if(strlen($token))
				{
					...
					else
					{
						...
						case 'zeros':
							...
							break;
						case 'spaces':
							$newstring.=str_pad($displayvalue,$displaypara, " ", STR_PAD_LEFT);
							break;
						case 'spell':
						...
						case 'explode':
							...
							break;
						case 'explode2':
							$sparr=explode(" ",trim($displayvalue),2);
							$newstring.=$sparr[intval($displaypara)];
							break;
						case 'datanastere':
							$an=substr($displayvalue,1,2);
							$mo=substr($displayvalue,3,2);
							$da=substr($displayvalue,5,2);
							if(substr($displayvalue,0,1)=="1" || substr($displayvalue,0,1)=="2")
								$newstring.=showDate('19'.$an."-".$mo.'-'.$da);
							else if(substr($displayvalue,0,1)=="3" || substr($displayvalue,0,1)=="4")
								$newstring.=showDate('18'.$an."-".$mo.'-'.$da);
							else if(substr($displayvalue,0,1)=="5" || substr($displayvalue,0,1)=="6")
								$newstring.=showDate('20'.$an."-".$mo.'-'.$da);
							else
								$newstring.=showDate('20'.$an."-".$mo.'-'.$da);
						break;
						case 'substr':
						...
						case 'pin':
							...
							break;
						case 'md':
							require_once("extern/Parsedown.php");
							$p=new Parsedown();
							$newstring.=$p->parse($displayvalue);
							break;
						case 'lang':
